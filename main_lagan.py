...
if __name__ == "__main__":
    ...
    if args.task != "rotmnist":
        if args.task == "traj_pred":
            ...
            if args.dataset_config is None:
                n_dim = 8  # 2 bodies, 4 coordinates per body
        ...
        # train_ds = CombinedDataset(unaug_train_ds, train_ds)
        n_dim = 784
        n_channel = 1
        d_input_size = 784
        ...
        def preprocess_to_torch(data):
            return torch.tensor(
                np.transpose(data["image"].numpy().squeeze(0), [0, 3, 1, 2]),
                dtype=torch.float32,
            ), torch.tensor(
                np.transpose(data["rot"].numpy().squeeze(0), [0, 3, 1, 2]),
                dtype=torch.float32,
            )
    ...
    if args.task == "top_tagging":
        discriminator = LieDiscriminatorEmb(d_input_size, n_class, emb_size).to(
            args.device
        )
    elif args.task == "rotmnist":
        discriminator = LieDiscriminatorLeNetEmb()
    else:
